...
- **트러블 슈팅** <br>
  - 차량 번호판 추출 과정에서 **오류** 발생하여 데이터 전처리 단계에서부터 **디버깅** 수행
  - **신규의 번호판 개정**으로 글자간의 간격이 달라 인식을 못하는 원인 확인
  - 데이터를 분류하고 각 모델별로 <b>파라미터 튜닝(epoch, batch size, learning rate)</b>을 통해 요구사항을 충족시켜 **인식 정확도 30% 향상**
  - 
